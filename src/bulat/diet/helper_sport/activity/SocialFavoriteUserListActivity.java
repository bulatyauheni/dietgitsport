...
import bulat.diet.helper_sport.adapter.UserArrayAdapter;
...
public class SocialFavoriteUserListActivity extends BaseActivity {
	...
	protected void onPause() {
		SaveUtils.saveScrollPosition(userList.getFirstVisiblePosition(),
				SocialFavoriteUserListActivity.this);
		super.onPause();
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Button favoriteButton = (Button) viewToLoad.findViewById(R.id.favoriteUsersButton);
...
*/
		...
		buttonProfile.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				intent.putExtra(SocialDishActivity.USERID,String.valueOf(SaveUtils.getUserUnicId(SocialFavoriteUserListActivity.this)));
				intent.putExtra(SocialDishActivity.USERNAME, SaveUtils.getNikName(SocialFavoriteUserListActivity.this));
				intent.putExtra(SocialDishActivity.USERWEIGHT, String.valueOf(SaveUtils.getRealWeight(SocialFavoriteUserListActivity.this)));
				intent.putExtra(SocialDishActivity.USERHEIGHT, String.valueOf((SaveUtils.getHeight(SocialFavoriteUserListActivity.this) + Info.MIN_HEIGHT)));
				if(!"0".equals( SaveUtils.getSex(SocialFavoriteUserListActivity.this))){
					intent.putExtra(SocialDishActivity.USERSEX, SocialFavoriteUserListActivity.this.getString(R.string.male));
				}else{
					intent.putExtra(SocialDishActivity.USERSEX, SocialFavoriteUserListActivity.this.getString(R.string.female));
				}
				intent.putExtra(SocialDishActivity.USERAGE, String.valueOf((SaveUtils.getAge(SocialFavoriteUserListActivity.this) + Info.MIN_AGE)));
				intent.putExtra(SocialDishActivity.USERSPORTING, SaveUtils.getActivity(SocialFavoriteUserListActivity.this));
	...
	protected void onResume() {
		...
		if (NetworkState.isOnline(getApplicationContext())) {
			...
			DialogUtils.showDialog(SocialFavoriteUserListActivity.this.getParent(),
					R.string.disonect);
		}
	}
	...
	private class LoadUsersTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if (NetworkState.isOnline(getApplicationContext())) {
				...
				parameters.append("?faworite_select=1&id="+SaveUtils.getUserUnicId(SocialFavoriteUserListActivity.this));
					...
					try {
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							if(!jsonObject.getString("user").equals(
									String.valueOf(SaveUtils.getUserUnicId(SocialFavoriteUserListActivity.this)))){
			...
			if (NetworkState.isOnline(getApplicationContext())) {
				...
				StringBuffer parameters = new StringBuffer("");
					try {
						parameters.append("?username="
										 + URLEncoder.encode(SaveUtils
												.getNikName(SocialFavoriteUserListActivity.this), "UTF-8"));
					...
					parameters.append("&weight="
							+(SaveUtils
							.getWeight(SocialFavoriteUserListActivity.this) + Info.MIN_WEIGHT));
					parameters.append("&high="
							+(SaveUtils
							.getHeight(SocialFavoriteUserListActivity.this) + Info.MIN_HEIGHT));
					parameters.append("&age="
							+(SaveUtils
							.getAge(SocialFavoriteUserListActivity.this) + Info.MIN_AGE));
					parameters.append("&sex="
							+(SaveUtils
							.getSex(SocialFavoriteUserListActivity.this)));
					parameters.append("&activity="
							+(SaveUtils
							.getActivity(SocialFavoriteUserListActivity.this)));
					...
					try {
						...
						if(Integer.valueOf(resultString) > 0)
							SaveUtils.setUserUnicId(SocialFavoriteUserListActivity.this, Integer.valueOf(resultString));
						}catch (Exception e) {
							e.printStackTrace();
						}
	...
	private void setAdapter() {
		...
		try {
			UserArrayAdapter da = new UserArrayAdapter(getApplicationContext(),
					...
					(SocialActivityGroup)SocialFavoriteUserListActivity.this.getParent());
			...
			buttonOk.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(userName.getText().length()<1){
						...
						SaveUtils.setNikName(SocialFavoriteUserListActivity.this, userName.getText().toString());
						dialog.cancel();
			...
			buttonOk.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(userName.getText().length()<1){
						...
						if(SaveUtils.getUserUnicId(SocialFavoriteUserListActivity.this) != 0){
							new SocialUpdater(SocialFavoriteUserListActivity.this).execute();
						}
					}
